...
{	
	...
	sky_image = cv_ptr_1->image;                
        ...
        {                                
                Mat path = skycam::RRT(map, car, goal, 10, scale, angle);   
                goal.x = -1;
                goal.y = -1;
                flag1 = false;                        
        }  
...
Mat skycam::RRT(Mat analyzedMap, Mat analyzedCar, Point goal, int beam, float scale, float angle)
{
        ...
        {                        
                ...
                if(cuadrante > 0)
                {
                        ...
                        {       
                                ...
                                ROS_INFO_STREAM("Point to transform: " << carVector[0] << ", " << carVector[1] << ", " << carVector[2]);
                                ...
                                pixel = analyzedMap.at<uchar>(analyzed.y, analyzed.x);
                                if(int(pixel) == 255)
                                {
                                        flag = false;
                                        xyz.clear();
                ...
                if(flag == true)
                {
                        ...
                        {
                                Point newPoint = Point(int(xyz[index][0] + planeOrigin.x), int(planeOrigin.y - xyz[index][1]));
                                pathVector.push_back(newPoint);
                                pathMap.at<uchar>(newPoint) = 255;
                        }
                        xyz.clear();
                        //whileFlag = false;  
                        currentAngle = atan2(Y,X) - 1.5708; 
