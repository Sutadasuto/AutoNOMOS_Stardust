...
{	
	...
	sky_image = cv_ptr_1->image;                
        ...
        if(flag == 2)
        {
                //newPoints = skycam::smoothPath(map, route);
                //for(int i = 0; i<newPoints.size(); i++)
                //{
                        //circle(sky_image, route[newPoints[i].x],1,CV_RGB(0,0,255),2);                 
                        //circle(sky_image, route[newPoints[i].y],1,CV_RGB(0,0,255),2);                 
                //}
                flag = 1;
        } 
        ...
        if(newPoints.size() > 0)
        {
                for (int point = 0; point < newPoints.size(); point++)
                {
                        circle(sky_image, newPoints[point],1,CV_RGB(0,0,0),2);                 
                }
        } 
        ...
        if(flag == 3)
        {
                ...
                else
                {
                        ...
                        //if(abs(IerrorVelocity) > speedLimit) IerrorVelocity = IerrorVelocity * speedLimit / abs(IerrorVelocity);
                        IerrorSteering = IerrorSteering + errorSteering;
                        //if(abs(IerrorSteering) > steerRange) IerrorSteering = IerrorSteering * steerRange / abs(IerrorSteering);
...
vector<Point> skycam::smoothPath(Mat analyzedMap, vector<Point> path)
{
        vector<Point> newRoute;
        vector<Point> destiny;
        vector<int> originalIndices = indices;
        destiny.push_back(path[0]);
        int ind;
        for (ind = 0; ind < originalIndices.size()-2; ind = ind + 2)
        //for (ind = 0; ind < 2; ind = ind + 2)
        {
                Mat pseudoCar = Mat::zeros(analyzedMap.rows, analyzedMap.cols, CV_8UC1);
                circle(pseudoCar, path[originalIndices[ind]],1,255,10);                
                Point p1 = path[originalIndices[ind]];
                Point p2 = path[originalIndices[ind] + 1];
                float emulatedAngle = -atan2(p2.y - p1.y, p2.x - p1.x) - 1.5708;
                newRoute = skycam::RRT(analyzedMap, pseudoCar, path[originalIndices[ind + 2]], 1000, scale, emulatedAngle);
                for(int i=1; i < newRoute.size(); i++)
                {
                        destiny.push_back(newRoute[i]/resizeScale);
                }
        }
        if(originalIndices.size()%2 == 0)
        {
                int begin = originalIndices[ind] + 1;
                for(int i = begin; i < path.size(); i++)
                {
                        destiny.push_back(path[i]);
                }
        }
        ...
        {       
                ...
                {       
                        ...
                        if(flag == true)
                        {
                                ...
                                if(pixel == 0)
                                {
                                        ...
                                        if(cv::norm(goal-candidatePosition) < minExpectedCost)
                                        {
                                                ...
                                                ROS_INFO_STREAM("minExpectedCost: " << minExpectedCost);
                                                xyzBest.clear();
                        ...
                        {               
                                ...
                                indices.push_back(pathVector.size()-1);                                
                                //ROS_INFO_STREAM("Indice: " << pathVector.size()-1);
                                xyz.clear();                                
                                //whileFlag = false;  
