...
vector<Point> smoothPath(Mat analyzedMap, vector<Point> path)
{
        ...
        float dir1, dir2;
        float threshold = 0.1;
        for(int i1 = 1; i1 < path.size() - 4; i1++)
        {
            for(int i2 = i1+2; i2 < path.size() - 2; i2++)
            {
                    X1 = path[i1].x - path[i1-1].x;
                    Y1 = path[i1].y - path[i1-1].y;
                    X2 = path[i2+1].x - path[i2].x;
                    Y2 = path[i2+1].y - path[i2].y;
            }    
        }
}
